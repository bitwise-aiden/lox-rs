...
impl<'a> Runner<'a> {
    pub fn new(stack: &'a mut Vec<Value>, allocator: &'a mut ObjAllocator, chunk: &'a Chunk) -> Self {
        Self {
            stack,
            allocator,
            chunk,
    ...
    fn run(&mut self) -> InterpretResult {
        loop {
            if debug::DEBUG_TRACE_EXECUTION {
                ...
                self.chunk.dissassemble_instruction(self.instruction_offset(), self.allocator);
            }
    ...
    fn read_string(&mut self) -> ObjRef<String> {
        match self.read_constant() {
            ...
            _ => { panic!("Constant is not String!") },
        }
    }
